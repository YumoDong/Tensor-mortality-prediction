...
rank=[1 1 1];
qq=10; %length of valiadation set
...
xx=15;
%In this file,CHE represents USA female, DNK represents USA male

CHE1=load('USA.mat'); %USA female
yyy=CHE1.USA;
xxx=yyy(:,female_d);   
che1=table2array(xxx); %table to array
zzz=zeros(ag,tt); 
for n=1:tt   
     zzz(1:ag,n)=che1(1+1887+20+(n-1)*111:1887+111*n-20,1);
end
...
for n=1:qq
    qqq(1:ag,n)=che1(1++1887+20+111*tt+(n-1)*111:1887+111*tt+111*n-20,1);
end
...
DNK1=load('USA.mat'); %USA male
yyy=DNK1.USA;
xxx=yyy(:,male_d);   
dnk1=table2array(xxx); %table to array
zzz=zeros(ag,tt); 
for n=1:tt   
    zzz(1:ag,n)=dnk1(1+1887+20+(n-1)*111:1887+111*n-20,1);
end
...
qqq =zeros(ag,qq); %Validation Matrix
for n=1:qq
    qqq(1:ag,n)=dnk1(1+20+1887+111*tt+(n-1)*111:111*tt+1887+111*n-20,1);
end
...
%We need to use 1950 to 1950+tt+qq-1 as our test set and fit a new Target tensor.
...
TT_real(:,:,1)=lnCHE_new; %This is before centering data
TT_real(:,:,2)=lnDNK_new;

%Mean of tt+qq
Mean_tq=[MeanCHE,MeanDNK]

%choose best fit rank[a b c] decomposition                 
 CP_newf=cell(trys,1);
parfor rr=1:trys;
    rng(rr)
...
clear m;
[Minnew,Seqnew]=min(RMSE_ranknew);% the best 
CP_new=CP_newf(Seqnew); % Now we get best rank decomposition.

cpnew_age=CP_new{1}.U{1};  % age vector  tensor
cpnew_country=CP_new{1}.U{3}; %gender  vector  tensor
cpnew_lambda=CP_new{1}.core; %the core tensor
...
end%we reverse-centering tesnor,which is log(mx) now.
clear m
FCD_cpnewb; 
...
DNK_new =zeros(ag,qq);
parfor n=1:qq
    CHE_new(:,n)=che1(1+20+1887+111*(tt+qq)+(n-1)*111:111*(tt+qq)+1887+111*n-20,1);
    DNK_new(:,n)=dnk1(1+20+1887+111*(tt+qq)+(n-1)*111:111*(tt+qq)+1887+111*n-20,1);
end
%
