...
rank=[8 14 9]; % the rank determined in the validation set.
qq=30; %length of valiadation set
...
UK_t=zeros(ag,tt); %(as ages * tt years)
for n=1:tt   
...
UK_v =zeros(ag,qq); %(ag ages * qq years) Validation Matrix
for n=1:qq
...
%We need to use 1922 to 1922+tt+qq-1 as our new training set and fit a new Target tensor.
lnSWE_new=[lnSWE_t,lnSWE_v]; %log(mx) of SWE  
...
% get means of the new training set
...
clear m
%
%We use centered log(mx) to consruct new Tensor.
...
% The real log(mx) before centering(91*(tt+qq)*cc)
...
for rr=1:trys;
    CP_ranknewf=double(CP_newf{rr,1});% imput every rank[a b c] decomposition after centering
   for ci=1:cc
    for m=1:tt+qq
...
%rank abc forecast
cpnew_age=CP_new{1}.U{1};  % age vector of rank 1 tensor
cpnew_country=CP_new{1}.U{3}; %country  vector of rank 1 tensor
cpnew_lambda=CP_new{1}.core; %the lambda
[cpnew_year,YMSE]=arimafunction(CP_new{1}.U{2},qq,arimapdq); % forecast year vectors ,random walk
...
end%we reverse-centering the rank 1 tesnor,which is log(mx) now.
clear m
FCD_cpnewb; 
%this is our forecast tensor which will be compared with the Lee-Carter model.

%Compare with the Lee-Carter model by RMSE(1922+tt+qq)
...
FRA_new =zeros(ag,qq);
parfor n=1:qq
    SWE_new(:,n)=swe1(sa+1+18981+111*(tt+qq)+(n-1)*111:18981+111*(tt+qq)+111*n-20,1);
    UK_new(:,n) =uk1(sa+1+111*(tt+qq)+(n-1)*111:111*(tt+qq)+111*n-20,1);
    CHE_new(:,n)=che1(sa+1+5106+111*(tt+qq)+(n-1)*111:5106+111*(tt+qq)+111*n-20,1);
    DNK_new(:,n)=dnk1(sa+1+9657+111*(tt+qq)+(n-1)*111:9657+111*(tt+qq)+111*n-20,1);
    ESP_new(:,n)=esp1(sa+1+1554+111*(tt+qq)+(n-1)*111:1554+111*(tt+qq)+111*n-20,1);
    FIN_new(:,n)=fin1(sa+1+4884+111*(tt+qq)+(n-1)*111:4884+111*(tt+qq)+111*n-20,1);
    ITA_new(:,n)=ita1(sa+1+5550+111*(tt+qq)+(n-1)*111:5550+111*(tt+qq)+111*n-20,1);
    NLD_new(:,n)=nld1(sa+1+7992+111*(tt+qq)+(n-1)*111:7992+111*(tt+qq)+111*n-20,1);
    NOR_new(:,n)=nor1(sa+1+8436+111*(tt+qq)+(n-1)*111:8436+111*(tt+qq)+111*n-20,1);
    FRA_new(:,n)=fra1(sa+1+11766+111*(tt+qq)+(n-1)*111:11766+111*(tt+qq)+111*n-20,1);
end
%
%impute real log(observations) 91*(1922+tt+qq to 2014)*3
...
%RMSE of overall forecast and country specific RMSE.
RMSE_cpnew= Realob_new-FCD_cpnewb; %get the residuals between the real and the forecst.
